...
/**
 * Genera una mappa completa con ZONE_MINIME zone casuali
 * Crea sia la mappa del Mondo Reale che quella del Soprasotto
 * Posiziona un Demotorzone in una zona casuale del Soprasotto
 */
...
/**
 * Inserisce manualmente una nuova zona in una posizione specifica
 * Chiede all'utente i dettagli della zona e la inserisce nella mappa
 * NOTA: il Demotorzone non puo' essere inserito manualmente (usare genera_mappa)
 */
static void inserisci_zona(void) {
    ...
    if (posizione == 1) {
        ...
        for (i = 1; i < posizione - 1 && current_mr != NULL; i++) {
            current_mr = current_mr->avanti;
            current_ss = current_ss->avanti;
        }
 ...
 * Rimuove sia la zona del Mondo Reale che quella corrispondente del Soprasotto
 */
...
/**
 * Stampa tutte le zone di una mappa
 * Permette di visualizzare il Mondo Reale o il Soprasotto
 */
...
/**
 * Stampa i dettagli di una singola zona e la sua corrispondente nell'altro mondo
 */
...
/**
 * Chiude la mappa e verifica che tutti i requisiti siano soddisfatti
 * Controlla che ci siano almeno ZONE_MINIME zone ed esattamente 1 Demotorzone
 */
...
/**
 * Funzione principale per l'impostazione del gioco
 * Permette di configurare i giocatori e creare la mappa
 */
 ...
 * Include statistiche, posizione e inventario
 * @param g Puntatore al giocatore da visualizzare
 */
...
/**
 * Stampa la descrizione della zona in cui si trova il giocatore
 * Include descrizione narrativa, presenza di nemici e oggetti
...
/**
 * Permette al giocatore di raccogliere un oggetto dalla zona corrente
 * Funziona solo nel Mondo Reale e se non ci sono nemici
 * @param g Puntatore al giocatore
 */
...
/**
 * Permette al giocatore di utilizzare un oggetto dall'inventario
 * Gli oggetti forniscono bonus permanenti alle statistiche
 * @param g Puntatore al giocatore
 */
...
/**
 * Controlla se c'e' un nemico nella zona corrente del giocatore
 * @param g Puntatore al giocatore
 * @return 1 se c'e' un nemico, 0 altrimenti
 */
...
/**
 * Fa avanzare il giocatore alla zona successiva
 * FIX: ora include il controllo del nemico direttamente nella funzione
 * @param g Puntatore al giocatore
 */
static void avanza(Giocatore* g) {
    ...
    /* FIX: controllo nemico difensivo interno */
    if (ha_nemico_zona(g)) {
...
/**
 * Fa indietreggiare il giocatore alla zona precedente
 * FIX: ora include il controllo del nemico direttamente nella funzione
 * @param g Puntatore al giocatore
 */
...
/**
 * Permette al giocatore di cambiare mondo attraverso un portale
 * Dal Mondo Reale al Soprasotto: possibile solo senza nemici
 * Dal Soprasotto al Mondo Reale: richiede tiro di fortuna (anche con nemico)
 * FIX: azzera il puntatore al mondo precedente per evitare puntatori "fantasma"
 * @param g Puntatore al giocatore
 * @return 1 se il cambio ha successo, 0 altrimenti
 */
...
/**
 * Inizializza le statistiche di un nemico in base al tipo
 * @param nemico   Tipo di nemico
 * @param hp       Puntatore ai punti vita
 * @param attacco  Puntatore all'attacco
 * @param difesa   Puntatore alla difesa
 */
static void inizializza_statistiche_nemico(Tipo_nemico nemico,
                                           int* hp,
                                           int* attacco,
                                           int* difesa) {
    switch (nemico) {
...
/**
 * Gestisce un combattimento completo tra il giocatore e un nemico
 * Sistema a turni con varie opzioni di attacco e difesa
 * FIX: gestione difesa temporanea tramite flag esplicito
 * FIX: controllo PV prima dell'attacco potenziato
 * @param g Puntatore al giocatore
 * @return  2 se ha sconfitto il Demotorzone (vittoria finale)
 *          1 se ha vinto il combattimento normale
 *          0 se non c'era nemico o errore
 *         -1 se il giocatore e' morto
 */
static int combatti_nemico(Giocatore* g) {
    ...
    int difesa_temporanea_attiva; /* FIX: flag esplicito per la difesa temporanea */
    ...
    while (hp_nemico > 0 && g->punti_vita > 0) {
        ...
        difesa_temporanea_attiva = 0; /* FIX: resetta flag a inizio turno giocatore */
        ...
        switch (scelta) {
            ...
            case 2:
                /* Attacco potenziato */
                /* FIX: controlla se il giocatore ha abbastanza PV */
                if (g->punti_vita <= COSTO_ATTACCO_POTENZIATO) {
            ...
            case 3:
                ...
                difesa_temporanea_attiva = 1; /* FIX: segna che e' attiva */
                break;
        ...
        if (hp_nemico <= 0) {
            /* FIX: rimuove difesa temporanea prima di uscire */
        ...
        /* FIX: ripristina difesa temporanea tramite flag, non tramite valore di scelta */
        if (difesa_temporanea_attiva) {
            g->difesa_psichica -= BONUS_DIFESA_TEMPORANEO;
...
/**
 * Funzione principale del gameplay
 * Gestisce il loop di gioco con i turni dei giocatori
 * FIX: ordine turni corretto con num_vivi_round fisso per round
 * FIX: idx_turno confrontato con num_vivi_round invece di giocatori_vivi
 * FIX: nel Soprasotto si puo' tentare la fuga anche con nemico presente
 */
void gioca(void) {
    int i;
    int scelta;
    ...
    while (1) {
        ...
        if (idx_turno == 0) {
            ...
            num_vivi_round = 0; /* FIX: ricalcolato UNA sola volta per round */
        ...
        /* FIX: usa num_vivi_round (fisso per il round), non giocatori_vivi (variabile) */
        ...
        if (giocatori[giocatore_corrente] == NULL) {
            idx_turno++;
            if (idx_turno >= num_vivi_round) { /* FIX: confronto con num_vivi_round */
                idx_turno = 0;
            }
            continue;
        }
        ...
        while (!turno_finito) {
            ...
            printf("5) Visualizza statistiche giocatore\n");
            ...
            switch (scelta) {
                ...
                case 3:
                    /* Cambia mondo */
                    /* FIX: nel Soprasotto si puo' tentare la fuga anche con nemico */
                    if (nemico_presente &&
                        giocatori[giocatore_corrente]->mondo == MONDO_REALE) {
                        ...
                        if (nemico_presente &&
                            giocatori[giocatore_corrente]->mondo == SOPRASOTTO) {
                            printf("\n>>> TENTATIVO DI FUGA DAL NEMICO! <<<\n");
                        }
                        if (cambia_mondo(giocatori[giocatore_corrente])) {
                            mossa_effettuata = 1;
                            nemico_presente  = ha_nemico_zona(giocatori[giocatore_corrente]);
                ...
                case 9:
                    if (nemico_presente && !appena_mosso_con_nemico) {
